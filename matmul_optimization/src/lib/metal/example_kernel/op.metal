...
CPU: 4000ms, ~60GOP/s
matmulInt4_SIMD_Q4Interleave(GPU): 2800 ms, ~85 GOP/s
...
kernel void matmulInt4_SIMD_Q4Interleave_unroll16(
                    device const packed_float4* inA,
                    device const packed_char4* inB, // column major
                    device float* result,
                    device const float* scales,
                    ...
                    uint2 id [[thread_position_in_grid]])
{
    ...
    for (uint i = 0; i < k; i += group_size){
        ...
        for (uint j = 0; j < group_size; j+= 16){
            ...
            packed_char4 packed_high_0 = (packed_8_0 >> 4) & lowMask;
            packed_char4 packed_high_1 = (packed_8_1 >> 4) & lowMask;
...
kernel void matmulInt4_SIMD_Q4Interleave_unroll32(
                    device const packed_float4* inA,
                    device const packed_char4* inB, // column major
                    device float* result,
                    device const float* scales,
                    ...
                    uint2 id [[thread_position_in_grid]])
{
    ...
    for (uint i = 0; i < k; i += group_size){
        ...
        for (uint j = 0; j < group_size; j+= 32){
            ...
            packed_char4 packed_high_0 = (packed_8_0 >> 4) & lowMask;
            packed_char4 packed_high_1 = (packed_8_1 >> 4) & lowMask;
            packed_char4 packed_high_2 = (packed_8_2 >> 4) & lowMask;
            packed_char4 packed_high_3 = (packed_8_3 >> 4) & lowMask;
