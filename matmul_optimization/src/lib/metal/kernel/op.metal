...
kernel void matmulUInt4_SIMD_Q4Interleave_unroll32_exp1(
                    device const packed_float4* inA,
                    device const packed_char4* inB, // column major
                    device float* result,
                    device const float* scales,
                    ...
                    uint2 id [[thread_position_in_grid]])
{
    ...
    for (uint i = 0; i < k; i += group_size){
        for (uint j = 0; j < group_size; j+= 32){
            ...
            packed_float4 inBlow_0 = packed_float4(packed_low_0);
            packed_float4 inBlow_1 = packed_float4(packed_low_1);
            packed_float4 inBlow_2 = packed_float4(packed_low_2);
            packed_float4 inBlow_3 = packed_float4(packed_low_3);

            packed_float4 inBhigh_0 = packed_float4(packed_high_0);
            packed_float4 inBhigh_1 = packed_float4(packed_high_1);
            packed_float4 inBhigh_2 = packed_float4(packed_high_2);
            packed_float4 inBhigh_3 = packed_float4(packed_high_3);

            sum4 += inBlow_0 + inBlow_1 + inBlow_2 + inBlow_3 + inBhigh_0 + inBhigh_1 + inBhigh_2 + inBhigh_3;
        }
    }
    float sum = sum4[0] + sum4[1] + sum4[2] + sum4[3];
    result[idy * n + idx] = sum;
...
kernel void matmulUInt4_SIMD_Q4Interleave_unroll32_exp2(
                    device const packed_float4* inA,
                    device const packed_char4* inB, // column major
                    device float* result,
                    device const float* scales,
...
kernel void matmulUInt4_SIMD_Q4Interleave_unroll32_exp4(
                    device const packed_float4* inA,
                    device const packed_char4* inB, // column major
                    device float* result,
                    ...
                    uint2 id [[thread_position_in_grid]])
{
    ...
    int s_idx = (idx * k) / group_size;
    for (uint i = 0; i < k; i += group_size){
        float scale = scales[s_idx++];
        packed_float4 scale4 = {scale, scale, scale, scale};
        for (uint j = 0; j < group_size; j+= 32){
            // sequential: (a, b), (c, d), (e, f), (g, h): 32 bit = 4xuint8
            // expected layout of inB: (a, e), (b, f), (c, g), (d, h)
...
kernel void matmulUInt4_SIMD_Q4Interleave_unroll32_exp5(
                    device const packed_float4* inA,
                    device const packed_char4* inB, // column major
                    device float* result,
                    device const half* scales,
                    ...
                    uint2 id [[thread_position_in_grid]])
{
    ...
    for (uint i = 0; i < k; i += group_size){
        ...
        for (uint j = 0; j < group_size; j+= 32){
            ...
            packed_float4 tmp = inBlow_0 + inBlow_1 + inBlow_2 + inBlow_3 + inBhigh_0 + inBhigh_1 + inBhigh_2 + inBhigh_3;
            group_sum = tmp[0] + tmp[1] + tmp[2] + tmp[3] + tmp[4];
        }
        sum += group_sum * scale;
    }
    result[idy * n + idx] = sum;
...
kernel void matmulUInt4_SIMD_Q4Interleave_unroll32_exp6(
                    device const packed_float4* inA,
                    device const packed_char4* inB, // column major
                    device float* result,
                    device const half* scales,
                    ...
                    uint2 id [[thread_position_in_grid]])
{
    ...
    for (uint i = 0; i < k; i += group_size){
        ...
        for (uint j = 0; j < group_size; j+= 32){
            ...
            packed_float4 tmp = inAlow_0 * inBlow_0 + inAlow_1 * inBlow_1 + inAlow_2 * inBlow_2 + inAlow_3 * inBlow_3;
            tmp += inAhigh_0 * inBhigh_0 + inAhigh_1 * inBhigh_1 + inAhigh_2 * inBhigh_2 + inAhigh_3 * inBhigh_3;
                    ...
                    uint2 id [[thread_position_in_grid]])
{
    ...
    for (uint i = 0; i < k; i += group_size){
        ...
        for (uint j = 0; j < group_size; j+= 32){
            ...
            // packed_char4 packed_8_col2_0 = inB[weight_col2_idx];
            // packed_char4 packed_8_col2_1 = inB[weight_col2_idx + 1];
            // packed_char4 packed_8_col2_2 = inB[weight_col2_idx + 2];
            // packed_char4 packed_8_col2_3 = inB[weight_col2_idx + 3];
            // packed_char4 packed_low_col2_0 = (packed_8_col2_0 & lowMask) - offsets;
            // packed_char4 packed_low_col2_1 = (packed_8_col2_1 & lowMask) - offsets;
            // packed_char4 packed_low_col2_2 = (packed_8_col2_2 & lowMask) - offsets;
            // packed_char4 packed_low_col2_3 = (packed_8_col2_3 & lowMask) - offsets;
            // packed_char4 packed_high_col2_0 = ((packed_8_col2_0 >> 4) & lowMask) - offsets;
            // packed_char4 packed_high_col2_1 = ((packed_8_col2_1 >> 4) & lowMask) - offsets;
            // packed_char4 packed_high_col2_2 = ((packed_8_col2_2 >> 4) & lowMask) - offsets;
            // packed_char4 packed_high_col2_3 = ((packed_8_col2_3 >> 4) & lowMask) - offsets;
            // packed_float4 inBlow_col2_0 = packed_float4(packed_low_col2_0) * scale4_col2;
            // packed_float4 inBlow_col2_1 = packed_float4(packed_low_col2_1) * scale4_col2;
            // packed_float4 inBlow_col2_2 = packed_float4(packed_low_col2_2) * scale4_col2;
            // packed_float4 inBlow_col2_3 = packed_float4(packed_low_col2_3) * scale4_col2;
            // packed_float4 inBhigh_col2_0 = packed_float4(packed_high_col2_0) * scale4_col2;
            // packed_float4 inBhigh_col2_1 = packed_float4(packed_high_col2_1) * scale4_col2;
            // packed_float4 inBhigh_col2_2 = packed_float4(packed_high_col2_2) * scale4_col2;
            // packed_float4 inBhigh_col2_3 = packed_float4(packed_high_col2_3) * scale4_col2;
            // sum4_col2 += inAlow_0 * inBlow_col2_0;
            // sum4_col2 += inAlow_1 * inBlow_col2_1;
            // sum4_col2 += inAlow_2 * inBlow_col2_2;
            // sum4_col2 += inAlow_3 * inBlow_col2_3;
            // sum4_col2 += inAhigh_0 * inBhigh_col2_0;
            // sum4_col2 += inAhigh_2 * inBhigh_col2_1;
            // sum4_col2 += inAhigh_2 * inBhigh_col2_2;
            // sum4_col2 += inAhigh_3 * inBhigh_col2_3;
        }
    }
    float sum = sum4[0] + sum4[1] + sum4[2] + sum4[3];
    result[idy * n + idx] = sum;
    // sum = sum4_col2[0] + sum4_col2[1] + sum4_col2[2] + sum4_col2[3];
    // result[idy * n + idx+1] = sum;
}
