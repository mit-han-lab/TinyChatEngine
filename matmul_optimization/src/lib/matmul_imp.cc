...
void MatmulOperator::CHECK_MATRICES(const struct matrix *A,
...
void MatmulOperator::mat_mul_multithreading(
...
void MatmulOperator::mat_mul_transpose(const struct matmul_params *params) {
  ...
  for (i = 0; i < C->row; i++)
    for (j = 0; j < C->column; j++) {
      float acc = 0;
      for (k = 0; k < A->column; k++)
        acc += data_A[i * A->column + k] * transpose_tmp[j * B->row + k];
      data_C[i * C->column + j] = acc;
    }
}
...
void MatmulOperator::mat_mul_transposed_fastover_column(
    const struct matmul_params *params) {
  int i, j, k;
  ...
  for (j = 0; j < num_thread; j++) {
    ...
    threads_args[j].B = B;
    threads_args[j].C = C;
    pthread_create(&thread_pool[j], NULL,
                   mat_mul_transposed_fastover_column_func, &threads_args[j]);
  }
  // Join threads
  for (j = 0; j < num_thread; j++) {
    pthread_join(thread_pool[j], NULL);
...
void MatmulOperator::mat_mul_transpose_simd(
    const struct matmul_params *params) {
  int i, j, k;
...
void MatmulOperator::evaluate(IMP_TYPE type,
                              const struct matmul_params *params) {
...
#ifdef ONEDNN_ENABLE
    for (int i = 0; i < RUNS; i++)
      this->mat_mul_onednn_int8(params);
#else
    fprintf(stderr, "ONEDNN not enable!\n");
    exit(-1);
#endif
  ...
  case INT8_BASELINE:
    ...
    for (int i = 0; i < RUNS; i++)
      this->naive_mat_mul_int8(params);
    break;
  case INT8_AVX:
    function_name = "mat_mul_avx_int8";
    for (int i = 0; i < RUNS; i++)
      this->mat_mul_avx_int8(params);
    break;
  case INT8_AVX_FAST:
    function_name = "mat_mul_avx_int8_fast";
    for (int i = 0; i < RUNS; i++)
      this->mat_mul_avx_int8_fast(params);
    break;
  ...
  case INT8_AVX_FAST_2x2_32UNROLL:
    ...
    for (int i = 0; i < RUNS; i++)
      this->mat_mul_avx_int8_fast_2x2_32unroll(params);
    break;
  case INT8_AVX_FAST_2x2_OMP:
    function_name = "mat_mul_avx_int8_fast_2x2_omp";
    for (int i = 0; i < RUNS; i++)
      this->mat_mul_avx_int8_fast_2x2_omp(params);
    break;
  default:
    break;
  }
  ...
  float GOPS = (float)((float)params->C.column * (float)params->C.row *
                       (float)params->B.row) *
               2 / (1000000000) * RUNS;
  std::cout << function_name << ": " << ms
            << " ms, GOPS/s:" << GOPS / ((float)ms / 1000) << std::endl;
}
        ...
        for (int j = tj; j < tj + BLK_SIZE; j += 4) {
          ...
          for (int k = 0; k < A->column; k += 4) {
            ...
            *acc0_fp_128 = _mm_add_ps(*acc0_fp_128, val);
            ...
            val = _mm_mul_ps(Aik_Aik3,
                             _mm_load_ps(&data_B[(j + 1) * B->column + k]));
            *acc1_fp_128 = _mm_add_ps(*acc1_fp_128, val);

            val = _mm_mul_ps(Aik_Aik3,
                             _mm_load_ps(&data_B[(j + 2) * B->column + k]));
            *acc2_fp_128 = _mm_add_ps(*acc2_fp_128, val);

            val = _mm_mul_ps(Aik_Aik3,
                             _mm_load_ps(&data_B[(j + 3) * B->column + k]));
