...
struct Fp32CLIPAttention_input {
    ...
    Fp32CLIPAttention_input(Matrix3D<float> hidden_states_, Matrix3D<float> attention_mask_, Matrix3D<float> past_key_,
...
class Fp32CLIPAttention {
   public:
    Fp32CLIPAttention(std::string param_path, const struct model_config config);
    Fp32CLIPAttention() {}
    static void initialized_memory(const struct model_config config);
    struct Fp32CLIPAttention_output forward(const struct Fp32CLIPAttention_input &input);
