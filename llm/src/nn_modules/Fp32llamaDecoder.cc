...
Matrix3D<float> Fp32llamaDecoder::prepare_decoder_attention_mask(int length, int past_length) {
    PROFILE_START("Fp32llamaDecoder::prepare_decoder_attention_mask");
    ...
    PROFILE_END("Fp32llamaDecoder::prepare_decoder_attention_mask");
    return causal_attention_mask;
}

Fp32llamaDecoder::Fp32llamaDecoder(std::string param_path, const struct model_config config) {
    ...
    for (int layer_idx = 0; layer_idx < config.num_layers; layer_idx++) {
        ...
        Fp32llamaDecoderLayer layer = Fp32llamaDecoderLayer(path, config, layer_idx);
