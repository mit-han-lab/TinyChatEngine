...
inline void transpose_1_2idx_float_threads(Matrix3D<float> &input, Matrix3D<float> &output) {
    PROFILE_START("Fp32CLIPAttention::transpose_1_2idx_float");
    ...
    if (input.m_dim_y == 1 || input.m_dim_z == 1) {
        ...
        for (int j = 0; j < num_thread; j++) {
            ...
            thread_pool[j] = std::thread(transpose_1_2idx_float_func, start_idx, end_idx, std::ref(input), std::ref(output));
        }
        // Join threads
        for (int j = 0; j < num_thread; j++) {
            thread_pool[j].join();
    ...
    PROFILE_END("Fp32CLIPAttention::transpose_1_2idx_float");
}


void Fp32CLIPAttention::initialized_memory(const struct model_config config) {
...
inline void Fp32CLIPAttention::shape(Matrix3D<float> unshape, Matrix3D<float> shaped, int sqlen) {
    PROFILE_START("Fp32CLIPAttention::shape");
...
inline void Fp32CLIPAttention::unshape(Matrix3D<float> shaped, Matrix3D<float> unshape, int sqlen) {
    ...
    PROFILE_END("Fp32CLIPAttention::unshpae");
}
...
struct Fp32CLIPAttention_output Fp32CLIPAttention::forward(const struct Fp32CLIPAttention_input &input) {
    PROFILE_START(profile_name);
    struct Fp32CLIPAttention_output output;
