...
//     PROFILE_END("Fp32GPTBigCodeDecoder::get_position_embed");
//     return pos_embeds;
// }

Fp32GPTBigCodeDecoder::Fp32GPTBigCodeDecoder(std::string param_path, const struct model_config config) {
    ...
    for (int layer_idx = 0; layer_idx < config.num_layers; layer_idx++) {
        ...
        Fp32GPTBigCodeDecoderLayer layer = Fp32GPTBigCodeDecoderLayer(path, config, layer_idx);
...
// GPTBigCodeDecoder:
struct Fp32GPTBigCodeDecoder_output Fp32GPTBigCodeDecoder::forward(const struct Fp32GPTBigCodeDecoder_input &input) {
    ...
    for (int i = 0; i < this->layers.size(); i++) {
        if (!input.has_past_keys_values) {
            struct Fp32GPTBigCodeDecoderLayer_input l_i = {hidden_states, causal_attention_mask};
            struct Fp32GPTBigCodeDecoderLayer_output l_o = this->layers[i].forward(l_i);
            ...
            struct Fp32GPTBigCodeDecoderLayer_input l_i = {hidden_states, causal_attention_mask, input.past_keys[i],
                                                    input.past_values[i]};
            struct Fp32GPTBigCodeDecoderLayer_output l_o = this->layers[i].forward(l_i);
            hidden_states = l_o.hidden_states;
            past_keys.push_back(l_o.past_key_value.first);
    ...
    struct Fp32GPTBigCodeDecoder_output output = {last_hidden_states, past_keys, past_values};
