...
inline void Fp32GPTBigCodeAttention::shpae(Matrix3D<float> unshape, Matrix3D<float> shaped, int sqlen) {
    PROFILE_START("Fp32GPTBigCodeAttention::shpae");
    ...
    PROFILE_END("Fp32GPTBigCodeAttention::shpae");
}
...
struct Fp32GPTBigCodeAttention_output Fp32GPTBigCodeAttention::forward(const struct Fp32GPTBigCodeAttention_input &input) {
    ...
    this->shpae(qkv_states_unshape, query_states, sqlen);
