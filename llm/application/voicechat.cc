...
int main(int argc, char* argv[]) {
   ...
   // Check for -concurrent argument only if -v is passed
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--concurrent") == 0 || strcmp(argv[i], "-c") == 0){
            concurrent = true;
    ...
    if (isLLaMA3(target_model)) {
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                LLaMA3Generate(m_path, &model, LLaMA_FP32, input, generation_config, "models/llama3_vocab.bin", true, true, concurrent);
            }
            ...
            while (true) {
                ...
                LLaMA3Generate(m_path, &model, LLaMA_INT4, input, generation_config, "models/llama3_vocab.bin", true, true, concurrent);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                LLaMAGenerate(m_path, &model, LLaMA_FP32, input, generation_config, "models/llama_vocab.bin", true, true, concurrent);
            }
        } else if (format_id == INT4) {
            m_path = "INT4/" + m_path;
            Int4LlamaForCausalLM model = Int4LlamaForCausalLM(m_path, get_opt_model_config(model_id));
            ...
            while (true) {
                ...
                LLaMAGenerate(m_path, &model, LLaMA_INT4, input, generation_config, "models/llama_vocab.bin", true, true, concurrent);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                LLaVAGenerate(llama_m_path, &llama_model, clip_m_path, &clip_model, LLaVA_INT4, input, img_path, generation_config, "models/llama_vocab.bin", true, true, false);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                LLaVAGenerate(llama_m_path, &llama_model, clip_m_path, &clip_model, VILA_INT4, input, img_path, generation_config, "models/llama_vocab.bin", true, true, true);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == FP32) {
            ...
            while (true) {
                ...
                MistralGenerate(m_path, &model, LLaMA_FP32, input, generation_config, "models/mistral_vocab.bin", true, true, concurrent);
            }
        } else if (format_id == INT4) {
            m_path = "INT4/" + m_path;
            Int4LlamaForCausalLM model = Int4LlamaForCausalLM(m_path, get_opt_model_config(model_id));
            ...
            while (true) {
                ...
                MistralGenerate(m_path, &model, LLaMA_INT4, input, generation_config, "models/mistral_vocab.bin", true, true, concurrent);
            }
        } else {
            std::cout << std::endl;
        ...
        if (format_id == QINT8) {
            ...
            std::vector<int> generated_ids =
                OPTGenerate(&model, OPT_INT8, input_ids, generation_config, &encoder, true, true, concurrent);
            ...
            // Generate
            std::vector<int> generated_ids =
                OPTGenerate(&model, OPT_FP32, input_ids, generation_config, &encoder, true, true, concurrent);
        } else if (format_id == INT4) {
            ...
            // Generate
            std::vector<int> generated_ids =
                OPTGenerate(&model, OPT_INT4, input_ids, generation_config, &encoder, true, true, concurrent);
        }
#endif  // QN_CUDA
    }
};
