...
void test_Fp32OPTDecoderLayer() {
    ...
    hidden_states.load("Fp32_layer_input.bin");
    Matrix3D<float> attention_mask(mem_buf.get_fpbuffer(sqlen * sqlen), 1, sqlen, sqlen);
    attention_mask.load("Fp32_layer_mask.bin");
    ...
    attn_outputGT.load("Fp32_layer_output.bin");
...
int main() { test_Fp32OPTDecoderLayer(); }
