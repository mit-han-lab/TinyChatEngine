...
__global__ void LlamaRMSNorm_half_kernel(const Matrix3D<half> x, const Matrix3D<float> weight, Matrix3D<half> output, float eps) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = blockIdx.y * blockDim.y + threadIdx.y;
...
void LlamaRMSNorm_half::forward(const Matrix3D<half> &x, Matrix3D<half> &output) {
    dim3 block(32, 32);
