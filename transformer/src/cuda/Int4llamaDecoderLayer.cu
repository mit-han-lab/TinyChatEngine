...
static half *final_layer_norm_arr;
static half *gate_proj_arr;
static half *up_proj_arr;
static half *down_proj_arr;
static half *hidden_states_arr;

__global__ void add_half(Matrix3D<half> a, Matrix3D<half> b, Matrix3D<half> c) {
...
__global__ void SiLuMul_half(Matrix3D<half> a, Matrix3D<half> b) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;

    if (i < a.length()) {
        half v = a.m_data[i];
        half silu_v = __hmul(v, __hdiv(__float2half(1.0f), __hadd(__float2half(1.0f), hexp(__hneg(v)))));
...
Int4llamaDecoderLayer::Int4llamaDecoderLayer(std::string param_path, const struct model_config config, int layer_idx) {
    if (layer_idx == 0) {
        allocate_aligned_memory_gpu(hidden_states_half_arr, config.max_sqlen * config.embed_dim * sizeof(half));
        allocate_aligned_memory_gpu(final_layer_norm_arr, config.max_sqlen * config.embed_dim * sizeof(half));
        allocate_aligned_memory_gpu(gate_proj_arr, config.max_sqlen * config.hidden_dim * sizeof(half));
        allocate_aligned_memory_gpu(up_proj_arr, config.max_sqlen * config.hidden_dim * sizeof(half));
        allocate_aligned_memory_gpu(down_proj_arr, config.max_sqlen * config.embed_dim * sizeof(half));
        allocate_aligned_memory_gpu(hidden_states_arr, config.max_sqlen * config.embed_dim * sizeof(half));
...
struct Int4llamaDecoderLayer_output Int4llamaDecoderLayer::forward(const struct Int4llamaDecoderLayer_input &input) {
    ...
    Matrix3D<half> hidden_states(hidden_states_arr, input.hidden_states.m_dim_x, input.hidden_states.m_dim_y,
    ...
    Matrix3D<half> residual_add(hidden_states_half_arr, input.hidden_states.m_dim_x, input.hidden_states.m_dim_y,
                                 input.hidden_states.m_dim_z);
    ...
    Matrix3D<half> post_attention_layernorm(final_layer_norm_arr, input.hidden_states.m_dim_x,
    ...
    Matrix3D<half> gate_proj(gate_proj_arr, input.hidden_states.m_dim_x, input.hidden_states.m_dim_y,
                              this->hidden_dim);
    this->gate_proj.forward(post_attention_layernorm, gate_proj);

    Matrix3D<half> up_proj(up_proj_arr, input.hidden_states.m_dim_x, input.hidden_states.m_dim_y, this->hidden_dim);
    ...
    Matrix3D<half> down_proj(down_proj_arr, input.hidden_states.m_dim_x, input.hidden_states.m_dim_y, this->embed_dim);
