...
Fp32llamaAttention::Fp32llamaAttention(std::string param_path, const struct model_config config)
{
    ...
    allocate_aligned_memory(q_weight, config.embed_dim * config.hidden_dim * sizeof(float));
    allocate_aligned_memory(k_weight, config.embed_dim * config.hidden_dim * sizeof(float));
    allocate_aligned_memory(v_weight, config.embed_dim * config.hidden_dim * sizeof(float));
    allocate_aligned_memory(o_weight, config.hidden_dim * config.embed_dim * sizeof(float));
    this->q_proj = Linear_FP(Matrix3D<float>(q_weight, 1, config.hidden_dim, config.embed_dim), param_path + "/weight.bin");
    this->k_proj = Linear_FP(Matrix3D<float>(k_weight, 1, config.hidden_dim, config.embed_dim), param_path + "/weight.bin");
    this->v_proj = Linear_FP(Matrix3D<float>(v_weight, 1, config.hidden_dim, config.embed_dim), param_path + "/weight.bin");
    this->o_proj = Linear_FP(Matrix3D<float>(o_weight, 1, config.embed_dim, config.hidden_dim), param_path + "/weight.bin");

    float *cos_buf, *sin_buf;
    allocate_aligned_memory(cos_buf, config.max_sqlen * (config.hidden_dim / config.num_heads) * sizeof(float));
    allocate_aligned_memory(sin_buf, config.max_sqlen * (config.hidden_dim / config.num_heads) * sizeof(float));
    Matrix3D<float> cos(cos_buf, 1, config.max_sqlen, (config.hidden_dim / config.num_heads));
    Matrix3D<float> sin(sin_buf, 1, config.max_sqlen, (config.hidden_dim / config.num_heads));
...
struct Fp32llamaAttention_output Fp32llamaAttention::forward(const struct Fp32llamaAttention_input &input)
{
    ...
    this->rotary_pos_emb.forward(key_states, value_states, start_idx, sqlen);
...
#else
    transpose_1_2idx_float(final_value_states, value_states_transpose);
#endif
