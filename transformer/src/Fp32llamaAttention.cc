...
void Fp32llamaAttention::initialized_memory(const struct model_config config) {
  allocate_aligned_memory(attn_weights_arr,
                          config.num_heads * config.max_sqlen *
                              config.max_sqlen * sizeof(float));
  allocate_aligned_memory(attn_output_fp_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  allocate_aligned_memory(attn_output_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  allocate_aligned_memory(attn_output_transpose_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  allocate_aligned_memory(key_states_unshape_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  allocate_aligned_memory(key_states_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  allocate_aligned_memory(value_states_unshape_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  allocate_aligned_memory(value_states_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  allocate_aligned_memory(query_states_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  allocate_aligned_memory(value_states_transpose_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  cache_num = new int[config.num_layers];
  for (int i = 0; i < config.num_layers; i++)
    cache_num[i] = 0;
  allocate_aligned_memory(query_states_unshape_arr,
                          config.max_sqlen * config.embed_dim * sizeof(float));
  key_states_arr_cache = new float **[config.num_layers];
  for (int i = 0; i < config.num_layers; ++i) {
    key_states_arr_cache[i] = new float *[2];
    for (int j = 0; j < 2; ++j) {
      allocate_aligned_memory(key_states_arr_cache[i][j], config.max_sqlen *
                                                              config.embed_dim *
                                                              sizeof(float));
    }
  }
  ...
  for (int i = 0; i < config.num_layers; ++i) {
    ...
    for (int j = 0; j < 2; ++j) {
      allocate_aligned_memory(value_states_arr_cache[i][j],
...
inline void Fp32llamaAttention::shape(Matrix3D<float> unshape,
                                      Matrix3D<float> shaped, int sqlen) {
...
inline void Fp32llamaAttention::unshape(Matrix3D<float> shaped,
                                        Matrix3D<float> unshape, int sqlen) {
...
Fp32llamaAttention::Fp32llamaAttention(std::string param_path,
                                       const struct model_config config) {
  float *q_weight, *k_weight, *v_weight, *o_weight;
  allocate_aligned_memory(q_weight,
                          config.embed_dim * config.hidden_dim * sizeof(float));
  allocate_aligned_memory(k_weight,
                          config.embed_dim * config.hidden_dim * sizeof(float));
  allocate_aligned_memory(v_weight,
                          config.embed_dim * config.hidden_dim * sizeof(float));
  allocate_aligned_memory(o_weight,
                          config.hidden_dim * config.embed_dim * sizeof(float));
  this->q_proj = Linear_FP(
      Matrix3D<float>(q_weight, 1, config.hidden_dim, config.embed_dim),
      param_path + "/q_proj/weight.bin");
  this->k_proj = Linear_FP(
      Matrix3D<float>(k_weight, 1, config.hidden_dim, config.embed_dim),
      param_path + "/k_proj/weight.bin");
  this->v_proj = Linear_FP(
      Matrix3D<float>(v_weight, 1, config.hidden_dim, config.embed_dim),
      param_path + "/v_proj/weight.bin");
  this->o_proj = Linear_FP(
      Matrix3D<float>(o_weight, 1, config.embed_dim, config.hidden_dim),
      param_path + "/o_proj/weight.bin");

  float *cos_buf, *sin_buf;
  allocate_aligned_memory(cos_buf, config.max_sqlen *
                                       (config.hidden_dim / config.num_heads) *
                                       sizeof(float));
  allocate_aligned_memory(sin_buf, config.max_sqlen *
                                       (config.hidden_dim / config.num_heads) *
                                       sizeof(float));
  Matrix3D<float> cos(cos_buf, 1, config.max_sqlen,
                      (config.hidden_dim / config.num_heads));
  Matrix3D<float> sin(sin_buf, 1, config.max_sqlen,
                      (config.hidden_dim / config.num_heads));
...
void *transpose_1_2idx_float_func(void *args_) {
  struct transpose_1_2idx_float_arg *args =
      (struct transpose_1_2idx_float_arg *)args_;
  ...
  for (int i = 0; i < input.m_dim_x; i++) {
    for (int j = 0; j < input.m_dim_y; j++) {
      for (int k = args->start_idx; k < args->end_idx; k++) {
        output.m_data[i * output.m_dim_y * output.m_dim_z + k * output.m_dim_z +
                      j] = input.m_data[i * input.m_dim_y * input.m_dim_z +
                                        j * input.m_dim_z + k];
      }
    }
  }
...
inline void transpose_1_2idx_float_threads(Matrix3D<float> &input,
  ...
  if (input.m_dim_y == 1 || input.m_dim_z == 1) {
    ...
    if (num_thread > loop_over_dim)
      num_thread = loop_over_dim;
    ...
    for (int j = 0; j < num_thread; j++) {
      ...
      pthread_create(&thread_pool[j], NULL, transpose_1_2idx_float_func,
                     &threads_args[j]);
    }
    // Join threads
    for (int j = 0; j < num_thread; j++) {
...
Fp32llamaAttention::forward(const struct Fp32llamaAttention_input &input) {
  PROFILE_START(profile_name);
  struct Fp32llamaAttention_output output;
  const int sqlen = input.hidden_states.m_dim_y,
            b = input.hidden_states.m_dim_x;
  assert(b == 1);

  Matrix3D<float> query_states_unshape(query_states_unshape_arr, b, sqlen,
                                       embed_dim);
  ...
  Matrix3D<float> query_states(query_states_arr, this->num_heads, sqlen,
                               this->head_dim);
  ...
  // self.head_dim).transpose(1, 2)
  Matrix3D<float> key_states_unshape(key_states_unshape_arr, b, sqlen,
                                     embed_dim);
  this->k_proj.forward(input.hidden_states, key_states_unshape);
  Matrix3D<float> key_states(key_states_arr, this->num_heads, sqlen,
                             this->head_dim);
  this->shape(key_states_unshape, key_states, sqlen);
  // value_states = self.v_proj(hidden_states).view(bsz, q_len, self.num_heads,
  // self.head_dim).transpose(1, 2)
  Matrix3D<float> value_states_unshape(value_states_unshape_arr, b, sqlen,
                                       embed_dim);
  this->v_proj.forward(input.hidden_states, value_states_unshape);
  Matrix3D<float> value_states(value_states_arr, this->num_heads, sqlen,
                               this->head_dim);
  ...
  if (input.has_past_key_value) {
    ...
    for (int i = 0; i < input.past_key.m_dim_x; i++) {
      memcpy(val_ptr, &input.past_value.m_data[past_block * i],
             past_block * sizeof(float));
      val_ptr += past_block;
      memcpy(val_ptr, &value_states.m_data[sq_block * i],
             sq_block * sizeof(float));
      val_ptr += sq_block;
      memcpy(key_ptr, &input.past_key.m_data[past_block * i],
             past_block * sizeof(float));
      key_ptr += past_block;
      memcpy(key_ptr, &key_states.m_data[sq_block * i],
             sq_block * sizeof(float));
      key_ptr += sq_block;
    }
  } else {
    // Put the data into the buffer
    memcpy(ret_value_states, value_states_arr,
           (this->num_heads * tgz * this->head_dim) * sizeof(float));
    memcpy(ret_key_states, key_states_arr,
           (this->num_heads * tgz * this->head_dim) * sizeof(float));
  }
  Matrix3D<float> final_value_states(ret_value_states, this->num_heads, tgz,
                                     this->head_dim);
  Matrix3D<float> final_key_states(ret_key_states, this->num_heads, tgz,
                                   this->head_dim);
  ...
  Matrix3D<float> value_states_transpose(value_states_transpose_arr,
                                         this->num_heads, this->head_dim, tgz);
  transpose_1_2idx_float_threads(final_value_states, value_states_transpose);

  Matrix3D<float> attn_output(attn_output_arr, this->num_heads, sqlen,
                              this->head_dim);
  ...
  Matrix3D<float> attn_output_transpose(attn_output_transpose_arr, 1, sqlen,
                                        this->num_heads * this->head_dim);
  this->unshape(attn_output, attn_output_transpose, sqlen);

  Matrix3D<float> attn_output_fp(attn_output_fp_arr, 1, sqlen,
                                 this->num_heads * this->head_dim);
