...
struct Fp32OPTAttention_input {
    ...
    Fp32OPTAttention_input(Matrix3D<float> hidden_states_, Matrix3D<float> attention_mask_, int layer_idx_)
        : hidden_states(hidden_states_), attention_mask(attention_mask_), layer_idx(layer_idx_) {}

    Fp32OPTAttention_input(Matrix3D<float> hidden_states_, Matrix3D<float> attention_mask_, Matrix3D<float> past_key_,
...
class Fp32OPTAttention {
   public:
    Fp32OPTAttention(std::string param_path, const struct model_config config);
    Fp32OPTAttention() {}
    static void initialized_memory(const struct model_config config);
    struct Fp32OPTAttention_output forward(const struct Fp32OPTAttention_input &input);
   ...
   private:
    ...
    std::string profile_name = "Fp32OPTAttention";
};
