...
class Linear_FP_int4 {
   public:
    Linear_FP_int4(Matrix3D<int32_t> weight_, std::string weight_path) : weight(weight_) {
        float *scale_ptr;
        float *offset_ptr;
        int32_t *zero_point_ptr;
        // length of int8_t weight = elements / 2
        // length of scales/offset = elements / QK = weight / (QK/2)
        // length of zero_point = 1
        assert((weight.m_dim_z * 8) % (QK) == 0);
        allocate_aligned_memory(scale_ptr, (this->weight.length() * 8 * sizeof(float)) / QK);
        allocate_aligned_memory(offset_ptr, (this->weight.length() * 8 * sizeof(float)) / QK);  // TODO: we don't need offset
        allocate_aligned_memory(zero_point_ptr, (this->weight.length() * sizeof(int32_t)) / QK);

        int x = this->weight.m_dim_x, y = this->weight.m_dim_y, z = (this->weight.m_dim_z * 8) / QK;
        scale = Matrix3D<float>(scale_ptr, x, y, z);
        offset = Matrix3D<float>(offset_ptr, x, y, z);
        zero_point = Matrix3D<int32_t>(zero_point_ptr, x, y, z / 8);
    ...
    void forward_fast(const Matrix3D<float> &x, Matrix3D<float> &output);
    Matrix3D<int32_t> weight;
    Matrix3D<float> scale;
    Matrix3D<float> offset;
    Matrix3D<int32_t> zero_point;
