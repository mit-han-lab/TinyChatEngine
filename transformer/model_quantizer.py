...
def _write_weight_to_file(prefix: str, qs, d, m, zp, is_lm_head=False, cuda_is_available=False):
    # Convert to bytes
...
# Quantize model
def _quantize_model(prefix, method="Q4_0", data_type="fp32", cuda_is_available=False):
    # Check model name
    ...
    if method not in quantization_funcs:
        raise ValueError("Invalid quantization method. Expected 'Q4_0', 'Q4_1', 'Q4_2', 'Q4_3' or 'Q4_4'.")
    ...
    print(f"Quantizing {model_name_size} with {method} method (cuda_is_available={cuda_is_available}). Original data type: {data_type}")
    ...
    if model_name == "OPT":
        ...
        qs, d, m, zp = quantize_method(weight_path, array_size, data_type, cuda_is_available, 1, array_size)  # TODO: fix this
        _write_weight_to_file(file_path, qs, d, m, zp, True, cuda_is_available)
    ...
    # LLaMA
        ...
        if file_size_bytes % bytes_per_element != 0:
            raise ValueError(f"Invalid file size of {weight_path}. Expected multiple of element number.")
        array_size = file_size_bytes // bytes_per_element
        qs, d, m, zp = quantize_method(weight_path, array_size, data_type, cuda_is_available, 4096, 32000)
        _write_weight_to_file(file_path, qs, d, m, zp, True, cuda_is_available)
        print("Quantization of lm_head finished.")
        ...
        for idx in range(layer_num):
            ...
            if file_size_bytes % bytes_per_element != 0:
                raise ValueError(f"Invalid file size of {weight_path}. Expected multiple of element number.")
            array_size = file_size_bytes // bytes_per_element
            qs, d, m, zp = quantize_method(weight_path, array_size, data_type, cuda_is_available, 11008, 4096)
            _write_weight_to_file(file_path, qs, d, m, zp, False, cuda_is_available)
            ...
            if file_size_bytes % bytes_per_element != 0:
                raise ValueError(f"Invalid file size of {weight_path}. Expected multiple of element number.")
            array_size = file_size_bytes // bytes_per_element
            qs, d, m, zp = quantize_method(weight_path, array_size, data_type, cuda_is_available, 4096, 11008)
            _write_weight_to_file(file_path, qs, d, m, zp, False, cuda_is_available)
    ...
    print(f"All the {data_type} weights of {model_name_size} has been quantized with {method} method (cuda_is_available={cuda_is_available}).")
...
def main():
    ...
    # _quantize_model(prefix=args.model_path, method=args.method, data_type=args.data_type, cuda_is_available=args.cuda_is_available, group_size=args.group_size)
    _quantize_model(prefix=args.model_path, method=args.method, data_type=args.data_type, cuda_is_available=args.cuda_is_available)
